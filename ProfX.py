...
TelePath = Connection(config.LOCALISATION.ANSWERBACK)
...
def connect(TrainingSystem=False):  
    if config.LIMS_USER:
        if config.LIMS_USER != "YOUR USERNAME HERE": 
            user = config.LIMS_USER
        else: 
    ...
    if config.LIMS_PW:
        if config.LIMS_PW == "YOUR PASSWORD HERE":
            pw = getpass.getpass()
        else:
            pw = config.LIMS_PW     
    else:
    ...
    TelePath.connect(IP=config.LIMS_IP, Port=config.LIMS_PORT, IBMUser=config.LOCALISATION.IBM_USER, Userprompt=None, PWPrompt=b"Password :", User=user, PW=pw)      
